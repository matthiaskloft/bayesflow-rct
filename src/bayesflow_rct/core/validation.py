...
def _compute_sbc_uniformity_tests(
    ranks: np.ndarray, n_post_draws: int, n_bins: int = 20
) -> dict[str, float]:
    """
    ...
    if expected_per_bin >= 5:
        chi2_stat, chi2_pvalue = chisquare(
            hist, f_exp=[expected_per_bin] * n_bins_actual
        )
    else:
...
def _compute_sbc_c2st(
    ranks: np.ndarray, n_post_draws: int, n_folds: int = 5, random_state: int = 42
) -> dict[str, float]:
    """
...
def make_bayesflow_infer_fn(
    ...
    def infer_fn(sim_data: dict[str, Any], n_post_draws: int) -> np.ndarray:
        ...
        # Run inference
        post_draws = bf_model.sample(
            conditions=data_cond, num_samples=int(n_post_draws)
        )
...
def compute_batch_metrics(
    ...
    for i, level in enumerate(full_coverage_levels):
        covered = (true_value >= lower_bounds_all[i]) & (
...
def aggregate_metrics(
    ...
    for cond_id in sim_metrics["id_cond"].unique():
        ...
        cal_errors = [
            cond_row[f"cal_error_{int(level_item * 100)}"]
            for level_item in coverage_levels
        ]
    ...
    cond_metrics = (
        pd.DataFrame(cond_metrics_list).sort_values("id_cond").reset_index(drop=True)
    )

    # Reorder columns
    sample_cond = condition_grid[0] if condition_grid else {}
    cond_param_cols = [
        k for k in sample_cond.keys() if k not in ("id", "id_cond", true_param_key)
    ]
    metric_cols = [
        "n_sims",
        "rmse",
    ...
    ordered_cols = (
        ["id_cond"] + cond_param_cols + ["true_value"] + metric_cols + coverage_cols
    )
    ...
    summary = {
        ...
        "overall_nrmse": np.sqrt(np.mean(squared_errors)) / prior_range
        if prior_range > 0
        else np.nan,
    ...
    condition_summary = (
        cond_metrics[summary_cols].agg(["mean", "std", "min", "median", "max"]).T
    )
...
def extract_calibration_metrics(
    ...
    summary = {
        ...
        # Error metrics
        "overall_rmse": np.sqrt(np.mean(squared_errors)),
        "overall_nrmse": np.sqrt(np.mean(squared_errors))
        / prior_range,  # BayesFlow style
        "overall_mae": np.mean(np.abs(errors)),
        "overall_bias": np.mean(errors),
        # Posterior quality
        "mean_contraction": np.mean(contraction),
    ...
    for level in coverage_levels:
        level_int = int(level * 100)
        summary[f"coverage_{level_int}"] = np.mean(
            coverage_results[f"covered_{level_int}"]
        )
    ...
    for level in full_coverage_levels:
        level_int = int(level * 100)
        coverage_profile[level] = float(
            np.mean(coverage_results[f"covered_{level_int}"])
        )
    summary["coverage_profile"] = coverage_profile
...
def run_validation_pipeline(
    ...
    if verbose:
        ...
        print("  Memory-efficient mode: metrics computed per-batch")
    ...
    if verbose:
        ...
        s = metrics["summary"]
        print(
            f"Summary: r={s['recovery_corr']:.4f}, R²={s['recovery_r2']:.4f}, "
